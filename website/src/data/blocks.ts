...
export const blocks: BlockItem[] = [
  {
    ...
    inputs: [
      'student-agent-pack startup workflow',
      'AGENTS.md, BOOTSTRAP.md, tools/, exercises/, and opencode.json',
      'exercises/01/hello_world.py + data/tiny.csv',
    ],
    ...
    handsOnActivity:
      'Start Jan, inspect the student-agent-pack, run hello_world.py, edit its TODO-STUDENT once, rerun, and verify one value in tiny.csv.',
    debriefPrompts: ['What failed first?', 'Which setup check increased confidence most?'],
    microArtifact: 'Startup evidence, hello-world output, manual verification, and TODO change note.',
    rubricSpecific: ['Setup evidence is concrete.', 'Hello-world run and manual check are explicit.'],
  ...
  },
  {
    ...
    lectureFocus: ['Human-in-the-loop review', 'MCP purpose, benefits, and risks', 'MCP architecture'],
    handsOnActivity:
      'Run mcp_tool.py, complete its timezone-pair and error-path TODO-STUDENT variants, and record a successful MCP call plus a handled error.',
    debriefPrompts: ['What MCP contract made integration easier?', 'Where did tool assumptions break?'],
    microArtifact: 'MCP success and handled-error evidence with sanity notes.',
